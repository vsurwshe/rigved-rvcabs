...
function LogoutCtrl($http, DotsCons, $location, authService, $scope, $localStorage) {
    ...
    if (token != undefined) {
        ...
        /* ------------------Logout Point-------------------- */
        $http.get(DotsCons.LOGOUT_POINT, config1).then(function (response) {$scope.RemoveCookie()}, function (reason) {$scope.RemoveCookie()});
        /* -------------------------------------------------- */
    } else {
        $scope.RemoveCookie();
    }
    ...
    $scope.tripBooking = function () {
        ...
        $http({
            ...
            $location.path('/managetrip')
                    ...
                    $.iaoAlert({
                        msg: "Trip Booking is not Successfully ..!",
                        type: "error",
                        mode: "dark",
                    })
        })
...
function manageTripController($scope, $http, DotsCons, authService, $localStorage, $q, $log, $location, GET_RIDE_LIST) {
    ...
    $scope.viewTripDetails = function (itemData) { $scope.tripModel = true; }
...
// this is traveling billing control
function travelBillingCtrl($scope, $http, DotsCons, $rootScope, authService, $localStorage, $location, $q, toaster, GET_CURRENT_DATA) {
    ...
    $scope.generateInvoice = function () {
        ...
        $http({
            ...
            if (response.data != null) { $rootScope.invoiceData = response.data; }
        }, function (errResponse) { return $q.reject(errResponse) })
...
function maintenanceReportCtrl($scope, $http, DotsCons, $rootScope, $route, authService, $localStorage, $location, $q, toaster, GET_CURRENT_DATA, $compile, DTOptionsBuilder, DTColumnBuilder) {
    ...
    $scope.pendingBillsDtColumns = [
        ...
        DTColumnBuilder.newColumn('totalAmount').withTitle('Amount').renderWith(function (data, type, full, meta) { return "₹&nbsp;" + data; }),
    ...
    $scope.invoiceImage=false;
    $scope.imageData;
    $scope.showImage= function (fileUrl) { getImage(fileUrl); }
    ...
    function getImage(url){
        ...
        return $http({
            ...
            $scope.imageData= URL.createObjectURL(response.data)
            $scope.invoiceImage= true;
            $scope.loading = false;
